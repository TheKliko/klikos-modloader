...
def start() -> None:
    ...
    SECTIONS: list[str] = ['Save & Exit', 'Mods', 'FastFlags', 'Settings', 'Help']
...
def mod_section() -> None:
    SECTIONS: list[str] = ['Go Back', 'Switch Mod Profiles', 'Configure Mod Profiles']
...
def mod_section_switch() -> None:
    ...
    while True:
        ...
        index, response = interface.select(
            ['Go Back'] + [profile if not profile == CURRENT_MOD_PROFILE else f'{CURRENT_MOD_PROFILE}{SELECTED_ITEM_INDICATOR}' for profile, mods in MOD_PROFILES.items()],
...
def configure_mod_profile(profile) -> None:
    ...
    while True:
        ...
        if 'back' in response.lower() or 'return' in response.lower():
            ...
            MODS: list = read_json.value(filepath=MOD_PROFILES_FILEPATH, key=profile)
            ...
            interface.text(
                ...
                + [f'[priority: {MODS.index(mod)}]: {mod}' if mod is not None else 'No mods found!' for mod in MODS]
                + [
                    '',
                    ''
                ]
            ...
            while True:
                ...
                index, response = interface.select(
                    ['Go Back']
                    + [mod for mod in installed_mods if not mod in mods],
            ...
            while True:
                ...
                interface.text(
                    [
                        'A lower number indicates a higher priority',
                        '(The highest priority is 0, the lowest is the number of mods in your mod profile)'
                    ]
                )

                index_old, response = interface.select(
                        ['Go Back']
                        + [mod for mod in mods],
                ...
                index_old -= 1  # Accommodate for the option 'Go Back'
                index_new = interface.select_number(min=0, prompt=f'Choose a new priority for {response} (current: {mods.index(response)})')
                mods.insert(index_new, mods.pop(index_old))

                write_json.value(filepath=MOD_PROFILES_FILEPATH, key=profile, value=mods)
                logging.info(f'Changed priority of {response} in {profile} ({index_old} -> {index_new})')
                time.sleep(0.15)
        ...
        for setting, info in settings.items():
            if response == setting:
                ...
                if info['type'] == 'toggle':
                    ...
                    # response = interface.confirm(prompt='Choose a new value')
