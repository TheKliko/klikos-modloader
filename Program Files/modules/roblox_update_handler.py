...
from modules.directory_functions import copy_directory, remove_directory
...
def get_latest_roblox_version() -> str:
    config_user_channel: dict = request_json(source=r'https://clientsettings.roblox.com/v2/user-channel')
    user_channel: str = get_json_value_from_input(config=config_user_channel, key='channelName')
    config: dict = request_json(source=(r'https://clientsettingscdn.roblox.com/v2/client-version/windowsplayer/channel/' + user_channel))
    value: str = get_json_value_from_input(config=config, key='clientVersionUpload')
    
    logging.debug(f'Detected user channel: {user_channel}')
    logging.debug(f'Retrieved roblox version: {value}')
...
def uninstall_roblox() -> None:
    try:
        remove_directory(source=version_directory, name='Version Folder')
    except Exception as e:
        logging.error(f'An unexpected {type(e).__name__} occured: {str(e)}')
