...
def transfer_data() -> None:  # Specific to transfering old data to v1.1.2
    ...
    logging.info('Transferring settings...')
    ...
    logging.info('Transferring mod profiles...')
    OLD_MOD_PROFILES_FILEPATH = os.path.join(CONFIG_DIRECTORY, 'mod_profiles.json')
    OLD_MOD_PROFILES: dict = read_json.complete(filepath=OLD_MOD_PROFILES_FILEPATH)
    if OLD_MOD_PROFILES:
        ...
        for profile, mods in OLD_MOD_PROFILES.items():
            if not isinstance(mods, dict):
                ...
                for priority, mod in enumerate(mods):
                    new_profile_format[priority] = mod
