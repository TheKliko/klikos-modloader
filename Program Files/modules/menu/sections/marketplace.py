...
def create_mod_frame(master, background: str|tuple[str,str], data: dict[str,str], index: int) -> None:
    ...
    ctk.CTkButton(
        ...
        cursor="hand2",
        command=lambda: download_mod(mod_id=id)
    ).grid(column=2, row=0, rowspan=3, padx=(0,16))


def download_mod(mod_id: str) -> None:
    filepath: str = os.path.join(Directory.program_files(), "modules", "functions", "mod_download.py")
    # command: str = "python \""+filepath+"\" "+mod_id
    new_command: str = "start cmd /c \"python \""+filepath+"\" "+mod_id+"\""
