...
def mods_section() -> None:
    ...
    while True:
        ...
        if 'back' in response_value.lower() or 'return' in response_value.lower():
            return None
        elif 'none' in response_value.lower():
            if interface.confirmation_prompt(prompt=f'Do you wish to select mod profile: {response_value}?'):
...
def fastflags_section() -> None:
    ...
    while True:
        ...
        if 'back' in response_value.lower() or 'return' in response_value.lower():
            ...
            if interface.confirmation_prompt(prompt=f'Do you wish to select fastflag profile: {response_value}?'):
                config: dict = variables.get('config_new')
