...
def load_licenses(master, width: int, background: str|tuple[str,str], index: int) -> None:
    ...
    box_width: int = 225
    scrollable_frame_height: int = 112
    box_padding: int = 8
    ...
    for i, item in enumerate(licenses):
        try:
            ...
            license_box = ctk.CTkFrame(
                ...
                width=box_width,
                fg_color=background,
                cursor="hand2" if url is not None else "arrow"
            )
            license_box.grid(column=i, row=0, sticky="nsew", padx=(0,8) if i != n else 0)
            license_box.grid_columnconfigure(0, weight=1, minsize=box_width)
            ...
            ctk.CTkLabel(
                ...
                justify="left",
                font=text_font
            ).grid(column=0, row=0, sticky="nsew", padx=box_padding, pady=(box_padding,0))
            ...
            ctk.CTkLabel(
                ...
                justify="left",
                font=text_font_small
            ).grid(column=0, row=1, sticky="nsew", padx=box_padding)
