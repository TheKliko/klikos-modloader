...
PLAYER_LAUNCH_ARGS: list[str] = ['launcher', 'roblox', 'play', 'launch', 'l', 'p', 'r'] # Don't know why I even bother accepting more than 1 argument for the same launch mode
STUDIO_LAUNCH_ARGS: list[str] = ['studio', 'create', 'roblox-studio', 'play-studio' , 'launch-studio', 'launcher-studio', 's', 'ls', 'ps', 'rs']
...
def main() -> None:
    ...
    try:
        ...
        if LAUNCH_MODE in PLAYER_LAUNCH_ARGS:
            launcher.run('roblox')

        elif LAUNCH_MODE in STUDIO_LAUNCH_ARGS:
            launcher.run('studio')
...
def get_launch_args() -> str:
    if len(sys.argv) > 1:
        launch_args: str = str(sys.argv[1]).lower().removeprefix('-').removeprefix('-')
        if launch_args in PLAYER_LAUNCH_ARGS and len(sys.argv) > 2 or launch_args in STUDIO_LAUNCH_ARGS and len(sys.argv) > 2:
            variables.set('roblox_launch_args', str(sys.argv[2]))
        return launch_args
    return None
