...
def check_for_mod_updates(mods: list[str], version: str) -> dict[str, list[Path]] | Literal[False]:
    ...
    for mod in mods:
        filepath: Path = Directory.MODS / mod / "info.json"
        ...
        try:
            ...
            if mod_hash not in result:
                result[mod_hash] = []
                result[mod_hash].append(Directory.MODS / mod)
