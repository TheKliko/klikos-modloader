...
class IntegrationsSection:
    ...
    def _load_title(self) -> None:
        ...
        ctk.CTkButton(buttons, text="Reset to default", image=restore_image, command=integrations.reset_all, width=1, anchor="w", compound=ctk.LEFT).grid(column=0, row=0, sticky="nsw")
    # endregion
    

    # region content
    def _load_content(self) -> None:
        items: dict[str, dict] = integrations.read_file()
        ...
        for i, (key, item) in enumerate(items.items()):
            ...
            ctk.CTkSwitch(
                ...
                command=lambda key=key, var=var: integrations.set_value(key, var.get())
